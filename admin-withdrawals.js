...
<button id="tabHistory" class="tab-withdraw-btn" data-tab="history"><i class="fas fa-clock-rotate-left"></i> Withdrawals History</button>
    ...
    <tr>
        ...
        <th style="padding: 10px 16px; color: #a8b4d0; font-weight: 600; font-size: 10px; text-transform: uppercase; letter-spacing: 0.3px; border-bottom: 1px solid rgba(255,255,255,0.04); background: rgba(10,14,28,0.3); text-align: left; min-width: 160px; width: 160px;">Remaining Balance</th>
...
<th style="padding: 10px 12px; color: #a8b4d0; font-weight: 600; font-size: 10px; text-transform: uppercase; letter-spacing: 0.3px; border-bottom: 1px solid rgba(255,255,255,0.04); background: rgba(10,14,28,0.3); text-align: left; min-width: 400px; width: 400px;">Wallet Address</th>
<th style="padding: 10px 12px; color: #a8b4d0; font-weight: 600; font-size: 10px; text-transform: uppercase; letter-spacing: 0.3px; border-bottom: 1px solid rgba(255,255,255,0.04); background: rgba(10,14,28,0.3); text-align: left; min-width: 150px; width: 150px;">User IP</th>
        ...
        <th style="padding: 10px 12px; color: #a8b4d0; font-weight: 600; font-size: 10px; text-transform: uppercase; letter-spacing: 0.3px; border-bottom: 1px solid rgba(255,255,255,0.04); background: rgba(10,14,28,0.3); text-align: left; min-width: 120px; width: 120px;">Actions</th>
    </tr>
</thead>
    ...
    <table class="data-table" style="width: 100%; border-collapse: collapse; font-size: 13px; min-width: 1050px;">
                        <thead>
                            <tr>
                                ...
                                <th style="padding: 14px 14px; color: #a8b4d0; font-weight: 600; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid rgba(255,255,255,0.04); background: rgba(10,14,28,0.3); text-align: left; min-width: 400px; width: 400px;">Wallet Address</th>
...
style.textContent = `
#page_withdrawals .tab-withdraw-btn {
...
#page_withdrawals .tab-withdraw-btn:hover {
    background: rgba(255,255,255,0.08);
    color: #e6edf5;
}
#page_withdrawals .tab-withdraw-btn.active {
    background: #2a3a5a;
    color: #e6edf5;
    border-color: #3a5a7a;
}
...
#page_withdrawals .wallet-address-text {
    font-size: 11px;
...
#page_withdrawals .copy-address-btn:hover {
    background: rgba(255,255,255,0.08);
    color: #b0c0da;
}
#page_withdrawals .currency-badge {
...
#page_withdrawals .currency-badge.usdt { background: rgba(38, 161, 123, 0.12); color: #26a17b; }
#page_withdrawals .currency-badge.eth { background: rgba(98, 126, 234, 0.12); color: #627eea; }
#page_withdrawals .currency-badge.btc { background: rgba(247, 147, 26, 0.12); color: #f7931a; }
#page_withdrawals .currency-badge.usdc { background: rgba(39, 117, 202, 0.12); color: #2775ca; }
#page_withdrawals .currency-badge i { font-size: 14px; }
#page_withdrawals .status-badge-approved { background: rgba(122, 208, 176, 0.10); color: #7ad0b0; padding: 2px 12px; border-radius: 40px; font-size: 11px; display: inline-block; }
#page_withdrawals .status-badge-rejected { background: rgba(232, 128, 128, 0.10); color: #e88080; padding: 2px 12px; border-radius: 40px; font-size: 11px; display: inline-block; }
#page_withdrawals .status-badge-pending { background: rgba(212, 192, 154, 0.10); color: #d4c09a; padding: 2px 12px; border-radius: 40px; font-size: 11px; display: inline-block; }
#page_withdrawals .btn-sm-action {
    padding: 4px 12px;
    font-size: 11px;
    border: none;
    border-radius: 40px;
...
#page_withdrawals .btn-sm-action:hover { opacity: 0.85; }
#page_withdrawals .btn-approve { background: rgba(122, 208, 176, 0.15); color: #7ad0b0; }
#page_withdrawals .btn-approve:hover { background: rgba(122, 208, 176, 0.25); }
#page_withdrawals .btn-reject { background: rgba(232, 128, 128, 0.15); color: #e88080; }
#page_withdrawals .btn-reject:hover { background: rgba(232, 128, 128, 0.25); }
#page_withdrawals .withdraw-panel { animation: fadeIn 0.3s ease; }
@keyframes fadeIn {
    from { opacity: 0; transform: translateY(8px); }
    to { opacity: 1; transform: translateY(0); }
}
#page_withdrawals .country-flag-img {
    width: 22px;
    height: 16px;
    border-radius: 2px;
...
#page_withdrawals .country-name {
    font-size: 12px;
    color: #c8d2e8;
    vertical-align: middle;
}
#page_withdrawals .custom-select-wrapper {
    position: relative;
    width: 100%;
    min-width: 160px;
}
#page_withdrawals .custom-select-display {
    display: flex;
    align-items: center;
...
#page_withdrawals .custom-select-display:hover {
    border-color: rgba(255,255,255,0.18);
    background: rgba(255,255,255,0.10);
}
#page_withdrawals .custom-select-display i {
    color: #5a6a82;
    font-size: 11px;
    transition: 0.25s ease;
    margin-left: 6px;
...
#page_withdrawals .custom-select-display i.fa-chevron-down {
    color: #5a6a82;
    font-size: 11px;
    margin-left: 6px;
}
#page_withdrawals .custom-select-dropdown {
    position: absolute;
    top: calc(100% + 6px);
    left: 0;
    right: 0;
...
#page_withdrawals .custom-select-dropdown.open {
    opacity: 1;
    visibility: visible;
    transform: translateY(0) scale(1);
    max-height: 240px;
}
#page_withdrawals .custom-select-dropdown::-webkit-scrollbar { width: 3px; }
#page_withdrawals .custom-select-dropdown::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.08); border-radius: 4px; }
#page_withdrawals .custom-select-options { padding: 4px 0; }
#page_withdrawals .custom-select-option {
    padding: 10px 18px;
    cursor: pointer;
...
#page_withdrawals .custom-select-option:hover {
    background: rgba(255,255,255,0.04);
    color: #e6edf5;
}
#page_withdrawals .custom-select-option.selected {
    background: rgba(255,255,255,0.02);
    color: #e6edf5;
}
#page_withdrawals .custom-select-option.selected::after {
    content: ' ✓';
    color: #6a8af0;
}
...
@media (max-width: 768px) {
    ...
    #page_withdrawals .wallet-address-text { font-size: 10px; }
    #page_withdrawals .stats-grid { grid-template-columns: repeat(2, 1fr) !important; }
    #page_withdrawals .tab-withdraw-btn { font-size: 12px; padding: 6px 14px; }
    #page_withdrawals .search-bar { flex-direction: column; align-items: stretch; }
    #page_withdrawals .search-bar input, #page_withdrawals .search-bar .custom-select-wrapper { width: 100% !important; min-width: unset; flex: 1 1 auto !important; }
    #page_withdrawals .custom-select-wrapper { min-width: unset !important; }
}
`;
...
addressCell.innerHTML = '<div class="wallet-address-wrapper"><span class="wallet-address-text">' + escapeHtml(address) + '</span>' + (address !== '-' ? '<button class="copy-address-btn" data-address="' + escapeHtml(address) + '"><i class="fas fa-copy"></i></button>' : '') + '</div>';
    ...
    // ===== Actions (索引 8) =====
    row.insertCell(8).innerHTML = '<div style="display:flex; gap:4px; flex-wrap:nowrap; align-items:center;"><button class="btn-sm-action btn-approve approve-withdraw" data-id="' + w.id + '" data-uid="' + w.uid + '" data-amt="' + w.amount + '" style="white-space:nowrap; font-size:10px; padding:4px 12px;"><i class="fas fa-check"></i> Approve</button><button class="btn-sm-action btn-reject reject-withdraw" data-id="' + w.id + '" data-uid="' + w.uid + '" data-amt="' + w.amount + '" style="white-space:nowrap; font-size:10px; padding:4px 12px;"><i class="fas fa-times"></i> Reject</button></div>';
}
