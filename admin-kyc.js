...
async function loadKycPending() {
    ...
    try {
        ...
        for (const [uid, items] of Object.entries(userGroups)) {
            ...
            row.insertCell(0).innerHTML = `<span class="badge" style="background: rgba(255,255,255,0.10); padding: 2px 12px; border-radius: 20px; font-size: 12px; color: #e8edf5; border: 1px solid rgba(255,255,255,0.12); font-weight: 600; letter-spacing: 0.3px;">${escapeHtml(uid)}</span>`;
