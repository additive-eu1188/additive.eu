...
async function loadEmailHistory() {
    ...
    try {
        ...
        for (const item of historyList) {
            ...
            row.insertCell(0).innerHTML = `<span style="font-size:12px; color:#b0c0da;">${escapeHtml(phone)}</span>`;
            row.insertCell(1).innerHTML = `<span style="font-weight:500; color:#d8e0f0;">${escapeHtml(item.email)}</span>`;
            row.insertCell(2).innerHTML = `<span class="badge" style="background: rgba(255,255,255,0.15); padding: 2px 12px; border-radius: 20px; font-size: 12px; color: #f0f4ff; border: 1px solid rgba(255,255,255,0.20); font-weight: 600; letter-spacing: 0.3px;">${escapeHtml(uid)}</span>`;
